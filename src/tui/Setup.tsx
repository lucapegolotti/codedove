...
export function Setup({ envPath, onComplete }: Props) {
  ...
  if (phase === "api") {
    ...
    return (
      <Box flexDirection="column" gap={1} padding={2}>
        <Text bold>claude-voice setup — API keys ({apiStep + 1}/{API_STEPS.length})</Text>
  ...
  return (
    <Box flexDirection="column" gap={1} padding={2}>
      <Text bold>claude-voice setup — preferences ({configStep + 1}/{CONFIG_STEPS.length})</Text>
      <Text dimColor>Saved to ~/.claude-voice/config.json</Text>
      <Box flexDirection="column" marginTop={1} gap={1}>
